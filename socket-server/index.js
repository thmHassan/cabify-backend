...
const autoDispatchRide = async ({
    ...
    try {
        ...
        try {
            ...
            if (!rows.length) {
                console.log(`[AutoDispatch] Booking #${bookingIdInt} not found`);
                return;
            }
            booking = rows[0];
            console.log(`[AutoDispatch] Booking: status="${booking.booking_status}" driver=${booking.driver} pickup_plot_id="${booking.pickup_plot_id}"`);
        } catch (e) {
            console.error(`[AutoDispatch] DB error (booking):`, e.message);
            return;
        }

        if (["cancelled", "completed"].includes(booking.booking_status)) {
            console.log(`[AutoDispatch]Status=${booking.booking_status}. Stop.`);
            return;
        }
        ...
        if (booking.booking_status === "pending_acceptance" && booking.driver && driverIndex === 0) {
            ...
            try {
                ...
                console.error(`[AutoDispatch] Reset error:`, e.message);
                return;
            }
        }
        ...
        if (!currentPlotId) {
            console.log(`[AutoDispatch] No plot on booking`);
            return;
        }

        const plotIdInt = parseInt(currentPlotId);
        const plotIdStr = String(currentPlotId);
        console.log(`[AutoDispatch] Plot: int=${plotIdInt} str="${plotIdStr}"`);

        // 3. Get idle drivers in plot
        let drivers = [];
        try {
            const [allInPlot] = await db.query(
                `SELECT id, name, driving_status, plot_id FROM drivers WHERE plot_id = ? OR plot_id = ?`,
                [plotIdStr, plotIdInt]
            );
            console.log(`[AutoDispatch] ALL drivers in plot ${plotIdInt}:`,
                allInPlot.length ? allInPlot.map(d => `#${d.id} ${d.name} [${d.driving_status}] plot_id="${d.plot_id}"`) : "NONE"
            );

            // Use both string and int to handle any column type
            const [idleRows] = await db.query(
                `SELECT * FROM drivers WHERE driving_status = 'idle' AND (plot_id = ? OR plot_id = ?)`,
                [plotIdStr, plotIdInt]
            );
            drivers = idleRows;
            console.log(`[AutoDispatch] Idle drivers in plot: ${drivers.length}`, drivers.map(d => `#${d.id} ${d.name}`));
        } catch (e) {
            console.error(`[AutoDispatch] DB error (drivers):`, e.message);
            return;
        }

        // 4. Nearest fallback 
        if (!drivers.length && driverIndex === 0) {
            console.log(`[AutoDispatch] No idle drivers in plot. Trying nearest (10km)...`);
            try {
                if (booking.pickup_point && booking.pickup_point.includes(',')) {
                    ...
                    console.log(`[AutoDispatch] Nearest idle (10km): ${nearestRows.length}`,
                        nearestRows.map(d => `#${d.id} ${d.name} (${Number(d.distance || 0).toFixed(2)}km)`)
                    );
                    if (nearestRows.length) drivers = nearestRows;
                }
            } catch (e) {
                console.error(`[AutoDispatch] Nearest query error:`, e.message);
            }
        }

        // 5. Backup plots
        if (!drivers.length || driverIndex >= drivers.length) {
            const plotKey = String(plotIdInt);
            if (!visitedPlots.includes(plotKey)) visitedPlots.push(plotKey);
            ...
            try {
                ...
                backupPlots = JSON.parse(plotRows[0]?.backup_plots || "[]");
            } catch (e) {
                console.error(`[AutoDispatch] Backup plots error:`, e.message);
            }

            console.log(`[AutoDispatch] Backup: ${JSON.stringify(backupPlots)} | Visited: ${JSON.stringify(visitedPlots)}`);
            const nextPlot = backupPlots.find(p => !visitedPlots.includes(String(p)));

            if (nextPlot) {
                console.log(`[AutoDispatch] → Backup plot ${nextPlot}`);
        ...
        // 6. Select & notify driver
        const driver = drivers[driverIndex];
        console.log(`[AutoDispatch] Driver selected: #${driver.id} "${driver.name}"`);
...
app.get("/debug/simulate-dispatch", async (req, res) => {
    const logs = [];
    const log = (msg, data = null) => {
        const entry = { msg, data };
        logs.push(entry);
        console.log("[SimDispatch]", msg, data ?? "");
    };

    try {
        const { booking_id, database } = req.query;
        if (!booking_id || !database) {
            return res.status(400).json({ error: "booking_id and database required" });
        }

        log(`=== SIMULATE DISPATCH START ===`);
        log(`booking_id=${booking_id} database=${database}`);

        // ── TEST 1: Try both DB name formats ─────────────────────────────────
        const dbFormats = [
            `tenant_${database}`,      // tenant_bahria_taxi34
            `tenant${database}`,       // tenantbahria_taxi34
            database,                  // bahria_taxi34 (raw)
        ];

        log(`Testing DB name formats:`, dbFormats);

        let workingDb = null;
        let workingDbName = null;

        for (const dbName of dbFormats) {
            try {
                const db = getConnection(dbName);
                const [testRows] = await db.query("SELECT 1 AS ok");
                if (testRows[0]?.ok === 1) {
                    log(`✅ DB connection works: "${dbName}"`);
                    workingDb = db;
                    workingDbName = dbName;
                    break;
                }
            } catch (e) {
                log(`❌ DB connection failed: "${dbName}"`, e.message);
            }
        }

        if (!workingDb) {
            return res.json({ success: false, logs, error: "No DB format works!" });
        }

        log(`Using DB: "${workingDbName}"`);

        // ── TEST 2: Load booking ──────────────────────────────────────────────
        const bookingIdInt = parseInt(booking_id);
        const [bookingRows] = await workingDb.query(
            "SELECT id, booking_status, driver, pickup_plot_id, destination_plot_id, pickup_point, offered_amount, booking_amount FROM bookings WHERE id = ?",
            [bookingIdInt]
        );

        if (!bookingRows.length) {
            log(`❌ Booking #${bookingIdInt} NOT FOUND in DB "${workingDbName}"`);
            return res.json({ success: false, logs, error: "Booking not found" });
        }

        const booking = bookingRows[0];
        log(`✅ Booking found`, booking);

        // ── TEST 3: Check booking status ──────────────────────────────────────
        log(`Booking status check: "${booking.booking_status}"`);
        if (["cancelled", "completed"].includes(booking.booking_status)) {
            log(`⏹ Would stop here: already ${booking.booking_status}`);
        } else if (booking.booking_status === "ongoing" && booking.driver) {
            log(`⏹ Would stop here: already ongoing`);
        } else {
            log(`✅ Status OK — dispatch would continue`);
        }

        // ── TEST 4: Resolve plot ──────────────────────────────────────────────
        const plotId = booking.pickup_plot_id || booking.destination_plot_id;
        log(`Plot ID from booking: "${plotId}"`);

        if (!plotId) {
            log(`❌ No plot on booking — dispatch would stop`);
            return res.json({ success: false, logs, error: "No plot" });
        }

        const plotIdInt = parseInt(plotId);
        const plotIdStr = String(plotId);

        // ── TEST 5: Query ALL drivers in plot (4 ways) ────────────────────────
        log(`Testing driver queries for plot_id="${plotIdStr}" / ${plotIdInt}...`);

        // Method A: string only
        const [methodA] = await workingDb.query(
            `SELECT id, name, driving_status, plot_id FROM drivers WHERE plot_id = ?`,
            [plotIdStr]
        );
        log(`Method A (plot_id = '${plotIdStr}'): ${methodA.length} drivers`, methodA.map(d => `#${d.id} ${d.name} [${d.driving_status}] plot=${d.plot_id}`));

        // Method B: integer only
        const [methodB] = await workingDb.query(
            `SELECT id, name, driving_status, plot_id FROM drivers WHERE plot_id = ?`,
            [plotIdInt]
        );
        log(`Method B (plot_id = ${plotIdInt}): ${methodB.length} drivers`, methodB.map(d => `#${d.id} ${d.name} [${d.driving_status}] plot=${d.plot_id}`));

        // Method C: CAST
        const [methodC] = await workingDb.query(
            `SELECT id, name, driving_status, plot_id FROM drivers WHERE CAST(plot_id AS UNSIGNED) = ?`,
            [plotIdInt]
        );
        log(`Method C (CAST): ${methodC.length} drivers`, methodC.map(d => `#${d.id} ${d.name} [${d.driving_status}] plot=${d.plot_id}`));

        // Method D: OR both
        const [methodD] = await workingDb.query(
            `SELECT id, name, driving_status, plot_id FROM drivers WHERE plot_id = ? OR plot_id = ?`,
            [plotIdStr, plotIdInt]
        );
        log(`Method D (OR both): ${methodD.length} drivers`, methodD.map(d => `#${d.id} ${d.name} [${d.driving_status}] plot=${d.plot_id}`));

        // ── TEST 6: Idle drivers only ─────────────────────────────────────────
        const [idleA] = await workingDb.query(
            `SELECT id, name, driving_status, plot_id FROM drivers WHERE driving_status = 'idle' AND plot_id = ?`,
            [plotIdStr]
        );
        log(`Idle drivers (string): ${idleA.length}`, idleA.map(d => `#${d.id} ${d.name}`));

        const [idleB] = await workingDb.query(
            `SELECT id, name, driving_status, plot_id FROM drivers WHERE driving_status = 'idle' AND (plot_id = ? OR plot_id = ?)`,
            [plotIdStr, plotIdInt]
        );
        log(`Idle drivers (OR): ${idleB.length}`, idleB.map(d => `#${d.id} ${d.name}`));

        // ── TEST 7: All drivers in DB ─────────────────────────────────────────
        const [allDrivers] = await workingDb.query(
            `SELECT id, name, driving_status, plot_id FROM drivers ORDER BY id`
        );
        log(`ALL drivers in DB: ${allDrivers.length}`, allDrivers.map(d => `#${d.id} ${d.name} [${d.driving_status}] plot=${d.plot_id}`));

        // ── TEST 8: Check socket map ──────────────────────────────────────────
        const socketDrivers = Array.from(driverSockets.entries()).map(([id, sid]) => ({ driver_id: id, socket_id: sid }));
        log(`Socket connected drivers: ${socketDrivers.length}`, socketDrivers);

        // ── TEST 9: Check if driver #6 socket exists ──────────────────────────
        const driver6Socket = driverSockets.get("6");
        const driver7Socket = driverSockets.get("7");
        log(`Driver #6 socket: ${driver6Socket ?? "NOT FOUND"}`);
        log(`Driver #7 socket: ${driver7Socket ?? "NOT FOUND"}`);

        // ── TEST 10: Describe drivers table ──────────────────────────────────
        try {
            const [cols] = await workingDb.query(`DESCRIBE drivers`);
            const plotCol = cols.find(c => c.Field === 'plot_id');
            log(`drivers.plot_id column type: "${plotCol?.Type}"`, plotCol);
        } catch (e) {
            log(`DESCRIBE drivers error:`, e.message);
        }

        // ── TEST 11: What req.tenantDb would be ───────────────────────────────
        log(`req.tenantDb (current middleware) = "tenant${database}"`);
        log(`Correct DB name found = "${workingDbName}"`);
        log(`Match: ${workingDbName === `tenant${database}` ? "✅ YES" : "❌ NO — middleware using wrong format!"}`);

        // ── CONCLUSION ────────────────────────────────────────────────────────
        const idleInPlot = idleB;
        const conclusion = {
            working_db_name: workingDbName,
            middleware_db_name: `tenant${database}`,
            db_name_correct: workingDbName === `tenant${database}`,
            booking_found: true,
            booking_status: booking.booking_status,
            plot_id: plotId,
            idle_drivers_in_plot: idleInPlot.length,
            socket_driver_6: !!driver6Socket,
            socket_driver_7: !!driver7Socket,
            would_dispatch_to: idleInPlot[0] ? `Driver #${idleInPlot[0].id} ${idleInPlot[0].name}` : "NOBODY — no idle drivers found",
            socket_would_fire: idleInPlot[0] ? !!driverSockets.get(String(idleInPlot[0].id)) : false
        };
        ...
        logs.push({ msg: `FATAL ERROR: ${err.message}`, stack: err.stack });
